  ...
  def convertRecord(record: SimulationRecord): Node = {
    ...
    <oai_dc:dc xmlns:oai_dc="http://www.openarchives.org/OAI/2.0/oai_dc/"
      ...
      <dc:description xml:lang="en">
        {record.englishDescrption}
      </dc:description>

      <!-- credits -->
      <dc:publisher>PhET Interactive Simulations</dc:publisher>
      <dc:creator>PhET Interactive Simulations</dc:creator>{record.authors.map(author => <dc:creator>
      {author}
    </dc:creator>)}<!-- rights -->
      <dc:rights>
        {record.englishRights}
      </dc:rights>
      ...
      <!-- mime types of content -->{record.mimeTypes.map(mimeType => <dc:format>
      {mimeType}
    </dc:format>)}<!-- available simulation languages -->{record.languages.map(language => <dc:language>
      {language}
    </dc:language>)}<!-- other titles -->{record.translatedTitles.filter(str => str.language != "en").map(str => <dc:title xml:lang={str.language}>
      {str.string}
    </dc:title>)}<!-- other descriptions -->{record.translatedDescriptions.filter(str => str.language != "en").map(str => <dc:description xml:lang={str.language}>
      {str.string}
    </dc:description>)}<!-- keywords -->{record.translatedTerms.map(term => term.map(str => <dc:subject xml:lang={str.language}>
      {str.string}
    </dc:subject>))}<!-- subjects from our "categories" -->{record.translatedCategories.map(term => term.map(str => <dc:subject xml:lang={str.language}>
      {str.string}
    </dc:subject>))}
    </oai_dc:dc>
  }
